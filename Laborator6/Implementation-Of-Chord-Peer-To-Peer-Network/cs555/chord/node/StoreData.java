...
//		m_fileName = m_filePath.split("/")[1];
		m_fileName = m_filePath.contains("\\") ?
				m_filePath.substring(m_filePath.lastIndexOf("\\") + 1) :
				m_filePath.substring(m_filePath.lastIndexOf("/") + 1);
		m_fileID = fileID;
	...
	{
		if (info == null || info.trim().isEmpty() || !info.contains(":")) {
			System.err.println("Invalid or empty random node info received: '" + info + "'");
			System.err.println("Cannot initiate lookup — no Chord peers available yet.");
			return;
		}

		String[] token = info.split(":");
		if (token.length < 3) {
			System.err.println("Malformed node info (expected at least IP:localPort:listenPort): " + info);
			return;
		}
