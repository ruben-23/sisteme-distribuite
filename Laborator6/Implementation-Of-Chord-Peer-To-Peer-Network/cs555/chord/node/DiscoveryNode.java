...
//		}
		...
		{
			if (peerNodeInfo == null || peerNodeInfo.isEmpty())
			{
				System.out.println("Warning: StoreData node requested random peer, but no peers registered yet. Ignoring request.");
				return; // Do NOT respond with empty info!
			}

			Object[] values = peerNodeInfo.values().toArray();
			String randomNodeInfo = (String) values[randomNode.nextInt(values.length)];
			...
			try {
				...
				System.out.println("Sent random node info to StoreData client: " + randomNodeInfo);
			} catch (Exception ex) {
				System.err.println("Failed to send random node response: " + ex.getMessage());
				ex.printStackTrace();
			} finally {
				if (msgSocket != null) {
					try { msgSocket.close(); } catch (IOException ignored) {}
				}
			}
		}
